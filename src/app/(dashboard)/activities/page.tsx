...
import { isProspectSuppressed } from '@/lib/prospectRules';
...
export default function ActivitiesPage() {
  ...
  useEffect(() => {
    let active = true;
    if (user) Promise.all([getActivities(user.uid), getProspects(user.uid), user.getIdToken().then(getProspectContactBarrier)])
      .then(([activityRecords, prospectRecords, barrier]) => { if (active) { setActivities(activityRecords); setProspects(prospectRecords); setContactGloballyBlocked(barrier.contactBlocked); } })
      .catch(() => { if (active) { setContactGloballyBlocked(true); setError('Contact history or suppression state could not be loaded. Outreach logs remain blocked.'); } });
    return () => { active = false; };
  }, [user]);
  ...
  async function save(event: React.FormEvent) {
    ...
    if (contactGloballyBlocked && ['email', 'call', 'proposal', 'meeting'].includes(type)) { setError('All prospect contact is blocked until unresolved suppression propagation is reconciled. Log an internal note instead.'); return; }
    if (isProspectSuppressed(prospect) && ['email', 'call', 'proposal'].includes(type)) { setError('This record is suppressed. Log an internal note instead of outreach.'); return; }
    ...
    try {
      ...
      if (['email', 'call'].includes(type)) await updateProspect(prospect.id, { lastContactDate: new Date().toISOString().slice(0, 10), nextFollowUpDate: followUpDate, contactAttempts: Number(prospect.contactAttempts || 0) + 1, status: prospect.status === 'ready_to_contact' ? 'contacted' : prospect.status }, idToken);
