...
import { createOutreachDraft, outreachDraftCopyBlockReasons, outreachTemplateNames, type OutreachTemplateKey } from '@/lib/outreachTemplates';
...
export default function SalesDeskPage() {
  ...
  const copyBlockReasons = useMemo(() => outreachDraftCopyBlockReasons(subject, body), [subject, body]);
  ...
  async function copyDraft() {
    ...
    if (copyBlockReasons.length > 0) { setError(`Draft remains blocked: ${copyBlockReasons.join(' ')}`); return; }
    try {
  ...
  return <div className="min-h-screen bg-slate-50 md:flex"><Sidebar /><main className="min-w-0 flex-1 p-4 md:p-8">
    ...
    <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><strong>Owner action required for every message.</strong> The desk only prepares copy. Resolve every bracketed fact and verify the recipient, category state, campaign status, responsible sender identity, reply method, physical mailing address, commercial disclosure, and opt-out line before copying. Do not use purchased lists, deceptive subjects, ringless voicemail, or recorded calls without separate legal review.</div>
    ...
    </aside><div className="rounded-xl border bg-white p-5 shadow-sm md:p-7"><h2 className="text-lg font-black">Editable draft</h2><label className="mt-5 block text-sm font-bold">Subject<input value={subject} onChange={(event) => setSubject(event.target.value)} disabled={contactGloballyBlocked} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal disabled:bg-slate-100" /></label><label className="mt-5 block text-sm font-bold">Body<textarea value={body} onChange={(event) => setBody(event.target.value)} rows={23} disabled={contactGloballyBlocked} className="mt-1 w-full rounded-lg border px-3 py-3 font-mono text-sm font-normal leading-6 disabled:bg-slate-100" /></label>{copyBlockReasons.length > 0 && <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><strong>Copy remains blocked.</strong><ul className="mt-2 list-disc space-y-1 pl-5">{copyBlockReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></div>}<div className="mt-5 flex flex-wrap items-center gap-3"><button disabled={contactGloballyBlocked || copyBlockReasons.length > 0} onClick={() => void copyDraft()} className="rounded-lg bg-blue-700 px-5 py-3 font-bold text-white disabled:opacity-40">Copy for manual review</button><span className="text-xs text-slate-500">No send button. No automatic activity log.</span></div></div></section>
