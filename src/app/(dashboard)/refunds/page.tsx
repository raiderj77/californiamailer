...
interface RefundRow {
  ...
  status: string;
  source: string | null;
  requiredFullRefund: boolean;
  integrityIssue: boolean;
  ownerRejectable: boolean;
...
interface LedgerState {
  payments: PaymentRow[];
  refunds: RefundRow[];
  ledgerIntegrity: { valid: boolean; activeIssueCount: number };
}

const PENDING_REFUND_REQUEST_STORAGE_PREFIX = 'californiamailer:refund-request:v1:';
const CLIENT_REQUEST_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_-]{15,99}$/;
...
export default function RefundReviewPage() {
  ...
  function pendingRefundRequestId(paymentId: string): string {
    const memoryRequestId = pendingRequestIds.current[paymentId];
    if (memoryRequestId && CLIENT_REQUEST_ID_PATTERN.test(memoryRequestId)) {
      return memoryRequestId;
    }
    const storageKey = `${PENDING_REFUND_REQUEST_STORAGE_PREFIX}${paymentId}`;
    try {
      const storedRequestId = window.localStorage.getItem(storageKey);
      if (storedRequestId && CLIENT_REQUEST_ID_PATTERN.test(storedRequestId)) {
        pendingRequestIds.current[paymentId] = storedRequestId;
        return storedRequestId;
      }
    } catch {
      // The in-memory key still makes retries safe while this page remains mounted.
    }
    const requestId = crypto.randomUUID();
    pendingRequestIds.current[paymentId] = requestId;
    try {
      window.localStorage.setItem(storageKey, requestId);
    } catch {
      // Browser storage can be unavailable; retain the same key in memory.
    }
    return requestId;
  }

  function clearPendingRefundRequest(paymentId: string) {
    delete pendingRequestIds.current[paymentId];
    try {
      window.localStorage.removeItem(`${PENDING_REFUND_REQUEST_STORAGE_PREFIX}${paymentId}`);
    } catch {
      // No storage cleanup is required when browser storage is unavailable.
    }
  }
    ...
    <main className="min-w-0 flex-1 p-4 md:p-8">
      <header className="mb-7 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">This ledger records refund obligations and keeps captured manual-review money out of cleared funding. It never calls Stripe or another provider. Submit refunds in the verified provider account, then record the reference here; only a signed webhook confirms completion.</p>
        </div>
        <button onClick={logout} className="rounded-lg border px-3 py-2 text-sm">Sign out</button>
      </header>
      {error && <Message error>{error}</Message>}
      {notice && <Message>{notice}</Message>}
      {ledger && !ledger.ledgerIntegrity.valid && <Message error>
        Refund ledger integrity is blocked. Refund records or linked payments have invalid campaign,
        payment, reservation, status, amount, or terminal provider-reconciliation evidence. Production
        and local refund mutations remain disabled until the underlying records are reconciled.
      </Message>}
      <div className="mb-7 grid gap-3 sm:grid-cols-3">
        ...
        <Stat label="Open amount" value={ledger?.ledgerIntegrity.valid ? formatCurrency(openObligations.reduce((total, refund) => total + (refund.amountCents || 0), 0)) : 'Blocked'} />
      </div>
        ...
        {!ledger ? <p className="mt-5 text-sm text-slate-500">Loading real payment records…</p> : ledger.payments.length === 0 ? <p className="mt-5 rounded-lg bg-slate-50 p-4 text-sm text-slate-600">No eligible payment records exist. No examples were inserted.</p> : <div className="mt-5 grid gap-4">
          {ledger.payments.map((payment) => <article key={payment.id} className="rounded-xl border p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[10rem_1fr_auto]">
              <label className="text-sm font-bold">Amount (USD)<input disabled={busyId === payment.id} type="number" min="0.01" step="0.01" value={amounts[payment.id] || ''} onChange={(event) => { clearPendingRefundRequest(payment.id); setAmounts((current) => ({ ...current, [payment.id]: event.target.value })); }} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal" /></label>
              <label className="text-sm font-bold">Reason<input disabled={busyId === payment.id} value={reasons[payment.id] || ''} onChange={(event) => { clearPendingRefundRequest(payment.id); setReasons((current) => ({ ...current, [payment.id]: event.target.value })); }} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal" /></label>
              <button disabled={busyId === payment.id || payment.availableToRequestCents < 1 || !amounts[payment.id] || (reasons[payment.id] || '').trim().length < 5} onClick={() => void act(payment.id, { action: 'request', paymentId: payment.id, requestId: pendingRefundRequestId(payment.id), amountCents: Math.round(Number(amounts[payment.id]) * 100), reason: reasons[payment.id] })} className="self-end rounded-lg bg-slate-950 px-4 py-2 font-bold text-white disabled:opacity-30">Record obligation</button>
            </div>
          </article>)}
        </div>}
      </section>
      ...
      <section className="mt-7 rounded-2xl border bg-white p-5 shadow-sm">
        ...
        {!ledger ? <p className="mt-5 text-sm text-slate-500">Loading real refund records…</p> : ledger.refunds.length === 0 ? <p className="mt-5 rounded-lg bg-slate-50 p-4 text-sm text-slate-600">No refund obligations have been recorded.</p> : <div className="mt-5 grid gap-4">
          {ledger.refunds.map((refund) => <article key={refund.id} className="rounded-xl border p-4">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-blue-700">{refund.status.replaceAll('_', ' ')}{refund.requiredFullRefund ? ' · required full refund' : ''}</p><h3 className="font-black">{refund.businessName}</h3><p className="mt-1 text-sm text-slate-600">{refund.reason}</p></div><div className="text-right"><div className="text-lg font-black">{refund.amountCents === null ? 'Invalid amount' : formatCurrency(refund.amountCents)}</div><div className="text-xs text-slate-500">{refund.createdAt ? new Date(refund.createdAt).toLocaleString() : 'Date unavailable'}</div></div></div>
            {refund.integrityIssue && <p className="mt-3 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-900">This record has invalid authoritative bindings, amounts, or terminal provider evidence. It blocks production and cannot be changed locally.</p>}
            ...
            {refund.status === 'requested' && !refund.integrityIssue && <div className="mt-4 flex flex-wrap gap-2"><button disabled={busyId === refund.id} onClick={() => void act(refund.id, { action: 'approve', refundId: refund.id })} className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white">Approve obligation</button>{refund.ownerRejectable ? <label className="flex min-w-64 flex-1 items-end gap-2 text-sm font-bold"><span className="sr-only">Rejection note for {refund.businessName}</span><input aria-label={`Rejection note for ${refund.businessName}`} placeholder="Reason for rejection" value={notes[refund.id] || ''} onChange={(event) => setNotes((current) => ({ ...current, [refund.id]: event.target.value }))} className="w-full rounded-lg border px-3 py-2 font-normal" /><button disabled={busyId === refund.id || (notes[refund.id] || '').trim().length < 3} onClick={() => void act(refund.id, { action: 'reject', refundId: refund.id, note: notes[refund.id] })} className="rounded-lg border border-rose-300 px-4 py-2 font-bold text-rose-800 disabled:opacity-30">Reject</button></label> : <p className="self-center text-sm font-bold text-amber-800">{refund.source === 'campaign_cancellation' ? 'Accepted cancellation policy: this obligation cannot be rejected locally.' : 'Provider-confirmed capture: this obligation cannot be rejected locally.'}</p>}</div>}
            {refund.status === 'covered' && <p className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-950">This mandatory balance is currently covered by other open refund obligations. Rejecting a covering obligation reopens this balance atomically.</p>}
            {refund.status === 'approved' && !refund.integrityIssue && <div className="mt-4 grid gap-2 md:grid-cols-[1fr_auto]"><label className="text-sm font-bold">Provider refund reference<input value={providerReferences[refund.id] || ''} onChange={(event) => setProviderReferences((current) => ({ ...current, [refund.id]: event.target.value }))} placeholder="Copy from the verified provider account" className="mt-1 w-full rounded-lg border px-3 py-2 font-normal" /></label><button disabled={busyId === refund.id || (providerReferences[refund.id] || '').trim().length < 5} onClick={() => void act(refund.id, { action: 'mark_submitted', refundId: refund.id, providerReference: providerReferences[refund.id] })} className="self-end rounded-lg bg-slate-950 px-4 py-2 font-bold text-white disabled:opacity-30">Record external submission</button></div>}
