...
function OpportunityDetail({ item, activities, stageDraft, setStageDraft, taskType, setTaskType, taskDate, setTaskDate, taskNote, setTaskNote, taskOutcome, setTaskOutcome, activityNote, setActivityNote, busy, tasksEnabled, mutate }: {
  item: CrmOpportunity;
  activities: CrmActivity[];
  ...
  return <>
    ...
    {item.source === 'quote_inquiry' && <div className="mt-5 rounded-lg border bg-slate-50 p-4 text-sm"><p>A quote request permits a response to that request only. Promotion creates an unqualified prospect and does not send a reply.</p><div className="mt-3 flex flex-wrap gap-2"><button disabled={busy || item.doNotContact || item.sourceStatus === 'dismissed'} onClick={() => void mutate({ action: 'promote_quote', inquiryId: item.recordId }, 'Quote inquiry promoted to a researching prospect. Verify the business before any manual response.')} className="rounded-lg bg-blue-700 px-3 py-2 font-bold text-white disabled:opacity-40">Promote to prospect</button><button disabled={busy || item.sourceStatus === 'dismissed'} onClick={() => void mutate({ action: 'set_quote_status', inquiryId: item.recordId, status: 'reviewed' }, 'Quote inquiry marked reviewed. No response was sent.')} className="rounded-lg border px-3 py-2 font-bold disabled:opacity-40">Mark reviewed</button><button disabled={busy || item.sourceStatus === 'dismissed'} onClick={() => void mutate({ action: 'set_quote_status', inquiryId: item.recordId, status: 'dismissed' }, 'Quote inquiry dismissed. No response was sent.')} className="rounded-lg border border-rose-300 px-3 py-2 font-bold text-rose-800 disabled:opacity-40">Dismiss</button></div></div>}

    {item.source === 'prospect' && !item.doNotContact && <>
      <section className="mt-5 border-t pt-5"><h4 className="font-black">Pipeline stage</h4><p className="mt-1 text-xs leading-5 text-slate-500">Manual prospect stages never establish a reservation or payment. Operational stages come only from server-owned workflows.</p>{['reservation', 'paid'].includes(item.stage) ? <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-bold text-blue-950">This stage is locked to the verified operational record.</div> : <div className="mt-3 flex gap-2"><select aria-label="New pipeline stage" value={stageDraft} onChange={(event) => setStageDraft(event.target.value as CrmStageId)} className="min-w-0 flex-1 rounded-lg border px-3 py-2 text-sm">{editableStages.map((stage) => <option key={stage.id} value={stage.id}>{stage.label}</option>)}</select><button disabled={busy || stageDraft === item.stage} onClick={() => void mutate({ action: 'set_stage', prospectId: item.recordId, stage: stageDraft }, 'Prospect stage updated. No external action was taken.')} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white disabled:opacity-40">Save</button></div>}</section>

      {tasksEnabled ? <section id="tasks" className="mt-5 border-t pt-5"><h4 className="font-black">Next owner action</h4><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><label className="grid gap-1 text-xs font-bold text-slate-600">Action<select value={taskType} onChange={(event) => setTaskType(event.target.value as CrmNextActionType)} className="rounded-lg border px-3 py-2 text-sm font-normal">{CRM_NEXT_ACTION_TYPES.map((action) => <option key={action.id} value={action.id}>{action.label}</option>)}</select></label><label className="grid gap-1 text-xs font-bold text-slate-600">Due date<input type="date" required value={taskDate} onChange={(event) => setTaskDate(event.target.value)} className="rounded-lg border px-3 py-2 text-sm font-normal" /></label></div><label className="mt-3 grid gap-1 text-xs font-bold text-slate-600">Task note<textarea rows={2} maxLength={300} value={taskNote} onChange={(event) => setTaskNote(event.target.value)} className="rounded-lg border px-3 py-2 text-sm font-normal" /></label>{taskType === 'prepare_sample' && <p className="mt-2 text-xs leading-5 text-slate-500">Record the verified need/source, requested California market, likely mail or partner-distribution format, route or partner-volume evidence, category fit, asset-rights basis, owner-controlled QR destination, cost unknowns, and reply permission. Scheduling this task does not send, order, publish, quote, or create a provider account.</p>}<button disabled={busy || !taskDate} onClick={() => void mutate({ action: 'schedule_next_action', prospectId: item.recordId, actionType: taskType, dueDate: taskDate, note: taskNote }, 'Internal next action scheduled. Nothing was sent.')} className="mt-3 w-full rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-40">Schedule internal task</button>{item.nextActionDate && <div className="mt-4 rounded-lg bg-blue-50 p-3 text-sm"><strong>Scheduled:</strong> {item.nextActionType ? humanize(item.nextActionType) : 'Owner task'} on {item.nextActionDate}{item.nextActionNote ? ` — ${item.nextActionNote}` : ''}<label className="mt-3 grid gap-1 text-xs font-bold text-slate-600">Factual completion note<textarea rows={2} maxLength={500} value={taskOutcome} onChange={(event) => setTaskOutcome(event.target.value)} className="rounded-lg border bg-white px-3 py-2 text-sm font-normal" /></label><button disabled={busy} onClick={() => void mutate({ action: 'complete_next_action', prospectId: item.recordId, outcome: taskOutcome }, 'Next action marked complete and added to the activity ledger.')} className="mt-2 rounded-lg border bg-white px-3 py-2 text-sm font-bold">Mark complete</button></div>}</section> : <div className="mt-5 rounded-lg border bg-slate-50 p-3 text-sm">Next-action tasks are hidden by the owner add-on setting.</div>}
