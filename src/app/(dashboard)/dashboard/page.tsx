...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { setProspects(await getProspects(user.uid)); } catch { nextErrors.push('Prospect database could not be read.'); }
    try {
  ...
  const followUps = useMemo(() => prospects.filter((item) => item.nextFollowUpDate && item.nextFollowUpDate <= today && !item.doNotContact)
    .sort((a, b) => String(a.nextFollowUpDate).localeCompare(String(b.nextFollowUpDate))).slice(0, 8), [prospects, today]);
  const ready = prospects.filter((item) => item.status === 'ready_to_contact' && contactGate(item).allowed).length;
