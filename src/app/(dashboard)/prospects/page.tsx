...
import { canonicalizeProspectSuppression, categoryConflict, contactGate, contactQueueStatuses, duplicateReasons, isCurrentProspectStatus, isLegacyOperationalProspectStatus, isProspectSuppressed, prospectFilterStatuses, prospectStatuses } from '@/lib/prospectRules';
import { getProspectContactBarrier, suppressProspectIdentity } from '@/lib/prospectSuppressionClient';
...
const emptyDraft: ProspectDraft = {
  ...
  proofStatus: 'not_started', renewalStatus: 'none', renewalDate: '', doNotContact: false, suppressed: false,
};
...
export default function ProspectsPage() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setError('Prospects or suppression state could not be loaded. Contact actions remain blocked.');
    }
  }, [user]);
  ...
  const selectableVisible = visible.filter((item) => !isProspectSuppressed(item));
  const today = new Date().toISOString().slice(0, 10);
  const dueCount = prospects.filter((item) => item.nextFollowUpDate && item.nextFollowUpDate <= today && !isProspectSuppressed(item)).length;
  ...
  async function save(event: React.FormEvent) {
    ...
    if (contactQueueStatuses.has(draft.status) && contactGloballyBlocked) { setError('Contact queues are globally blocked until unresolved suppression propagation is reconciled.'); return; }
    if (!draft.businessName.trim()) { setError('Business name is required.'); return; }
    if (editing && isProspectSuppressed(editing) && !isProspectSuppressed(draft)) {
      setError('Suppression cannot be reopened in this release. A separate audited renewed-consent workflow is required.');
      return;
    }
    const requestsNewSuppression = isProspectSuppressed(draft) && !isProspectSuppressed(editing || {});
    if (!editing && requestsNewSuppression) {
      setError('A new DNC decision cannot be created in the browser. Save the factual prospect first, then apply identity-wide suppression from its review form.');
      return;
    }
    const savedDraft = canonicalizeProspectSuppression(draft);
    const suppressionIdentity = {
      businessName: savedDraft.businessName,
      email: savedDraft.email,
      phone: savedDraft.phone,
      website: savedDraft.website || '',
    };
    const needsServerSuppression = Boolean(editing && (requestsNewSuppression || isProspectSuppressed(editing)));
    setBusy(true);
    let suppressionApplied = false;
    try {
      const idToken = await user.getIdToken();
      if (editing?.id) {
        let suppressionNotice = '';
        if (needsServerSuppression) {
          const result = await suppressProspectIdentity(editing.id, idToken, suppressionIdentity);
          suppressionApplied = true;
          if (result.propagationStatus.startsWith('unresolved_')) setContactGloballyBlocked(true);
          suppressionNotice = result.propagationStatus.startsWith('unresolved_')
            ? 'Identity-wide DNC was applied to the target and discovered matches, but propagation coverage remains unresolved. Review the audit record.'
            : requestsNewSuppression
              ? `Identity-wide DNC applied to ${result.suppressedCount} matching prospect or source record(s).`
              : `Suppressed identity verified across ${result.suppressedCount} matching prospect or source record(s).`;
        }
        await updateProspect(editing.id, savedDraft, idToken);
        ...
        setNotice(suppressionNotice || 'Prospect saved. No message was sent.');
      } else { await addProspect({ ...savedDraft, userId: user.uid }, idToken); }
      setShowForm(false); setEditing(null); if (!editing) setNotice('Prospect saved. No message was sent.'); await reload();
    } catch (caught) {
      setError(suppressionApplied
        ? 'Identity-wide DNC was applied, but a later factual edit or activity-log write failed. Refresh before continuing.'
        : caught instanceof Error ? caught.message : 'The prospect could not be saved. No message was sent.');
      if (suppressionApplied) await reload();
    }
  ...
  async function applyBulkStatus() {
    ...
    const suppressedItems = items.filter(isProspectSuppressed);
    if (suppressedItems.length) { setError('Suppressed records cannot receive bulk status changes. Nothing was changed.'); return; }
    if (contactQueueStatuses.has(bulkStatus)) {
      if (contactGloballyBlocked) { setError('Contact queues are globally blocked until unresolved suppression propagation is reconciled. Nothing was changed.'); return; }
    ...
    try {
      const idToken = await user.getIdToken();
      if (bulkStatus === 'do_not_contact') {
        const results = [];
        for (const item of items) {
          if (item.id) results.push(await suppressProspectIdentity(item.id, idToken));
        }
        await Promise.all(items.flatMap((item) => item.id ? [
          addActivity({ prospectId: item.id, prospectName: item.businessName, type: 'note', description: 'Bulk identity-wide do-not-contact suppression applied.', outcome: 'recorded', followUpDate: item.nextFollowUpDate || '', userId: user.uid }),
        ] : []));
        const unresolved = results.some((result) => result.propagationStatus.startsWith('unresolved_'));
        setNotice(unresolved
          ? 'Bulk DNC applied to the selected targets and discovered matches; at least one identity lookup remains unresolved. Review audit records.'
          : `${items.length} selected target(s) received identity-wide DNC. No message was sent.`);
      } else {
        await Promise.all(items.flatMap((item) => item.id ? [
          updateProspect(item.id, { status: bulkStatus }, idToken),
        ...
        setNotice(`${items.length} record(s) updated. No message was sent.`);
      }
      setSelected(new Set()); await reload();
    } catch { setError(bulkStatus === 'do_not_contact' ? 'Bulk DNC did not fully complete. Some targets may already be suppressed; refresh and review audit records.' : 'Bulk update failed. Review the records before trying again.'); }
    finally { setBusy(false); }
  }
  ...
  function exportRows() {
    downloadCSV(filtered.map((item) => ({
      ...
      'Renewal Status': item.renewalStatus || '', 'Renewal Date': item.renewalDate || '', 'Do Not Contact': isProspectSuppressed(item) ? 'yes' : 'no', Notes: item.notes,
  ...
  return <div className="min-h-screen bg-slate-50 md:flex"><Sidebar /><main className="min-w-0 flex-1 p-4 md:p-8">
    ...
    <div className="mb-5 grid gap-3 md:grid-cols-3"><Stat label="Records" value={prospects.length} /><Stat label="Follow-ups due" value={contactGloballyBlocked ? 0 : dueCount} /><Stat label="Ready to contact" value={contactGloballyBlocked ? 0 : prospects.filter((item) => item.status === 'ready_to_contact' && contactGate(item).allowed).length} /></div>
    {contactGloballyBlocked && <div role="alert" className="mb-5 rounded-xl border border-rose-300 bg-rose-50 p-4 text-sm font-bold text-rose-900">All prospect contact actions are blocked because suppression propagation is unresolved. Research, factual corrections, internal notes, and additional DNC decisions remain available; outreach and contact queues do not.</div>}
    ...
    <section className="overflow-hidden rounded-xl border bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-slate-100 text-xs uppercase tracking-wide text-slate-600"><tr><th className="p-3"><input aria-label="Select visible prospects" type="checkbox" checked={selectableVisible.length > 0 && selectableVisible.every((item) => item.id && selected.has(item.id))} onChange={(event) => setSelected((current) => { const next = new Set(current); selectableVisible.forEach((item) => { if (!item.id) return; if (event.target.checked) next.add(item.id); else next.delete(item.id); }); return next; })} /></th><th className="p-3">Business</th><th className="p-3">Qualification</th><th className="p-3">Status</th><th className="p-3">Contact</th><th className="p-3">Follow-up</th><th className="p-3">Campaign / category</th><th className="p-3">Action</th></tr></thead><tbody className="divide-y">{visible.map((item) => {
      const gate = contactGate(item); const conflicts = categoryConflict(prospects, item, item.id); const suppressed = isProspectSuppressed(item);
      return <tr key={item.id} className={suppressed ? 'bg-rose-50' : ''}><td className="p-3"><input aria-label={`Select ${item.businessName}`} type="checkbox" disabled={suppressed} checked={Boolean(item.id && selected.has(item.id))} onChange={() => item.id && setSelected((current) => { const next = new Set(current); if (next.has(item.id!)) next.delete(item.id!); else next.add(item.id!); return next; })} /></td><td className="p-3"><div className="font-bold text-slate-950">{item.businessName}</div><div className="text-slate-500">{item.businessCategory || 'Category unverified'} · {item.city || 'City unverified'}</div>{item.website && <a href={item.website.startsWith('http') ? item.website : `https://${item.website}`} target="_blank" rel="noreferrer" className="text-xs text-blue-700 underline">Official site</a>}</td><td className="p-3"><Badge tone={item.qualificationStatus === 'qualified' ? 'green' : item.qualificationStatus === 'disqualified' ? 'red' : 'yellow'}>{statusLabel(item.qualificationStatus || 'verify')}</Badge>{!gate.allowed && !suppressed && <div className="mt-1 max-w-48 text-xs text-slate-500">Missing: {gate.missing.join(', ')}</div>}</td><td className="p-3"><Badge tone={suppressed ? 'red' : isLegacyOperationalProspectStatus(item.status) ? 'yellow' : 'blue'}>{suppressed ? 'Do Not Contact' : prospectStatusLabel(item.status)}</Badge></td><td className="p-3"><div>{item.contactName || 'Decision maker unknown'}</div><div className="text-slate-500">{item.email || 'Email not verified'}</div><div className="text-xs text-slate-400">{item.contactAttempts || 0} attempt(s)</div></td><td className="p-3">{item.nextFollowUpDate || '—'}</td><td className="p-3"><div>{item.campaignId || 'Not assigned'}</div><div className={conflicts.length ? 'font-bold text-amber-700' : 'text-slate-500'}>{item.businessCategory || 'No category'}{conflicts.length ? ` · ${conflicts.length} possible prospect-note conflict` : ''}</div></td><td className="p-3"><button onClick={() => void openEdit(item)} className="font-bold text-blue-700 underline">{suppressed ? 'View locked record' : 'Review'}</button></td></tr>;
...
function ProspectForm({ draft, setDraft, editing, history, prospects, busy, close, save }: { draft: ProspectDraft; setDraft: React.Dispatch<React.SetStateAction<ProspectDraft>>; editing: Prospect | null; history: Activity[]; prospects: Prospect[]; busy: boolean; close: () => void; save: (event: React.FormEvent) => void }) {
  ...
  const suppressionLocked = Boolean(editing && isProspectSuppressed(editing));
  const draftSuppressed = isProspectSuppressed(draft);
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-3 md:p-8" role="dialog" aria-modal="true" aria-labelledby="prospect-form-title"><form onSubmit={save} className="mx-auto max-w-5xl rounded-2xl bg-white p-5 shadow-2xl md:p-8"><div className="mb-6 flex items-start justify-between gap-4"><div><h2 id="prospect-form-title" className="text-2xl font-black">{editing ? 'Review prospect' : 'Add researched business'}</h2><p className="mt-1 text-sm text-slate-500">Save public-source evidence and qualification before outreach. This form never sends a message.</p></div><button type="button" onClick={close} className="rounded border px-3 py-2">Close</button></div>
    ...
    <Section title="Decision maker and follow-up"><div className="grid gap-4 md:grid-cols-3"><Input label="Contact name" value={draft.contactName} onChange={(e) => set('contactName', e.target.value)} /><Input label="Role" value={draft.contactRole || ''} onChange={(e) => set('contactRole', e.target.value)} /><Input label="Email" type="email" value={draft.email} onChange={(e) => set('email', e.target.value)} /><Input label="Phone" value={draft.phone} onChange={(e) => set('phone', e.target.value)} /><Input label="Last contact" type="date" value={draft.lastContactDate || ''} onChange={(e) => set('lastContactDate', e.target.value)} /><Input label="Next follow-up" type="date" value={draft.nextFollowUpDate || ''} onChange={(e) => set('nextFollowUpDate', e.target.value)} /><Input label="Contact attempts" type="number" min="0" value={draft.contactAttempts ?? 0} onChange={(e) => set('contactAttempts', Number(e.target.value))} /><label className="block text-sm font-bold text-slate-700">Lead status<select value={draft.status} disabled={draftSuppressed} onChange={(e) => { if (!isCurrentProspectStatus(e.target.value)) return; setDraft((current) => e.target.value === 'do_not_contact' ? canonicalizeProspectSuppression({ ...current, status: 'do_not_contact' }) : { ...current, status: e.target.value as Prospect['status'] }); }} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal text-slate-950 disabled:bg-slate-100">{legacyStatus && <option value={draft.status} disabled>{prospectStatusLabel(draft.status)}</option>}{prospectStatuses.map((value) => <option key={value} value={value}>{statusLabel(value)}</option>)}</select></label><label className="flex items-center gap-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-bold text-rose-900"><input type="checkbox" checked={draftSuppressed} disabled={draftSuppressed} onChange={(e) => { if (e.target.checked) setDraft((current) => canonicalizeProspectSuppression(current)); }} />{draftSuppressed ? 'Do not contact — locked' : 'Do not contact / suppress'}</label></div>{draftSuppressed && <div className="mt-3 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-900">Suppression cannot be cleared here. This release has no reopening UI or API; renewed consent requires a separate audited workflow.</div>}{suppressionLocked && <p className="mt-2 text-xs text-slate-500">Other factual fields and internal notes may still be corrected while contact status remains locked.</p>}</Section>
