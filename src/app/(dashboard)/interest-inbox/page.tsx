  ...
  return <div className="min-h-screen bg-slate-50 md:flex"><Sidebar /><main className="min-w-0 flex-1 p-4 md:p-8">
    ...
    {!items.length && !error ? <p className="rounded-xl border bg-white p-8 text-center text-slate-500">No real interest records exist. No samples were created.</p> : <div className="grid gap-5">
      {items.map((item) => {
        ...
        const blocked = ['dismissed', 'do_not_contact'].includes(item.status) || inactiveModel;
        return <article key={item.id} className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-wide text-blue-700">{item.publicReference} · {item.status.replaceAll('_', ' ')}</div><h2 className="mt-1 text-xl font-black">{item.businessName}</h2><p className="text-sm text-slate-600">{item.contactName} · <a className="underline" href={`mailto:${item.email}`}>{item.email}</a>{item.phone ? ` · ${item.phone}` : ''}</p></div><div className="text-right text-sm"><strong>{category?.name || item.categorySlug}</strong><div>{item.placementSize} placement</div><div className="text-slate-500">{item.createdAt ? new Date(item.createdAt).toLocaleString() : 'Date unavailable'}</div></div></div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button disabled={busyId === item.id || blocked} onClick={() => void act(item, { action: 'create_invite', expiresHours: 72, sensitiveCategoryApproved: Boolean(sensitiveApprovals[item.id]) })} className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-40">Issue 72-hour code</button>
            ...
            <button disabled={busyId === item.id} onClick={() => void act(item, { action: 'set_status', status: 'reviewed' })} className="rounded-lg border px-4 py-2 text-sm">Mark reviewed</button>
            <button disabled={busyId === item.id} onClick={() => void act(item, { action: 'set_status', status: 'dismissed' })} className="rounded-lg border px-4 py-2 text-sm">Dismiss</button>
            <button disabled={busyId === item.id} onClick={() => void act(item, { action: 'set_status', status: 'do_not_contact' })} className="rounded-lg border border-rose-300 px-4 py-2 text-sm font-bold text-rose-800">Do not contact</button>
