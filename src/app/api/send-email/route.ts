...
import { normalizeEmail } from '@/lib/prospectIdentity';
import {
...
export async function POST(request: NextRequest) {
  ...
  const requestHash = sha256(JSON.stringify({ ...quoteDetails, email: normalizeEmail(email) }));
  ...
  const emailGuardRef = db.collection('publicrequestguards').doc(`quote-email-${sha256(normalizeEmail(email))}`);
