  ...
  try {
    ...
    if (parsed.data.action in evidenceActions) {
      ...
      const lifecycle = await db.runTransaction(async (transaction) => {
        ...
        if (['schedule_for_print', 'record_printed'].includes(action)
          && (latest.ownerPrintApproved !== true || !latest.printReadyAt)) {
    ...
    if (parsed.data.action === 'cancel_campaign') {
      ...
      const cancellation = await db.runTransaction(async (transaction) => {
        ...
        if (payments.docs.some((document) => ['disputed', 'manual_review'].includes(String(document.data().status)))) {
          return { success: false as const, reason: 'payment_review' as const };
        }
        const openByPayment = new Map<string, number>();
        for (const document of refunds.docs) {
          if (!['requested', 'approved', 'submitted'].includes(String(document.data().status))) continue;
          const paymentId = String(document.data().paymentId || '');
          openByPayment.set(paymentId, (openByPayment.get(paymentId) || 0) + Number(document.data().amountCents || 0));
        }
        let remainingClearedCents = 0;
        const obligations: Array<{ id: string; data: DocumentData }> = [];
        for (const payment of payments.docs) {
          const data = payment.data();
          if (!['cleared', 'partially_refunded'].includes(String(data.status))) continue;
          const refundedCents = Number(data.refundedCents || 0);
          const net = Math.max(0, Number(data.amountCents || 0) - refundedCents);
          remainingClearedCents += net;
          const uncovered = Math.max(0, net - Number(openByPayment.get(payment.id) || 0));
          if (uncovered < 1) continue;
          obligations.push({
            id: `${FOUNDING_CAMPAIGN.id}__cancel__${payment.id}__${refundedCents}`,
            data: {
              campaignId: FOUNDING_CAMPAIGN.id,
    ...
    if (parsed.data.action === 'close_cancelled') {
      ...
      const closing = await db.runTransaction(async (transaction) => {
        ...
        const unresolved = refunds.docs.some((document) => ['requested', 'approved', 'submitted'].includes(String(document.data().status)));
        const unsafePayment = payments.docs.some((document) => ['disputed', 'manual_review', 'pending'].includes(String(document.data().status)));
        const netCleared = payments.docs.reduce((total, document) => {
          const data = document.data();
          return ['cleared', 'partially_refunded'].includes(String(data.status))
            ? total + Math.max(0, Number(data.amountCents || 0) - Number(data.refundedCents || 0))
            : total;
        }, 0);
