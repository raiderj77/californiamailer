...
export async function GET(request: NextRequest) {
  try {
    ...
    const [prospectSnapshot, activitySnapshot, interestSnapshot, quoteSnapshot, settingsSnapshot, barrierSnapshot] = await Promise.all([
  ...
  try {
    ...
    if (input.action === 'promote_quote') {
      ...
      await db.runTransaction(async (transaction) => {
        const [inquirySnapshot, barrierSnapshot] = await Promise.all([
        ...
        if (clean(inquiry.status) === 'dismissed' || isRecordSuppressed(inquiry)) {
        ...
        if (isProspectContactBarrierActive(barrierSnapshot.data())) {
          throw new CrmConflictError('Prospect contact is globally blocked until unresolved suppression propagation is reconciled.', 409);
        }
        const prospectSnapshot = await transaction.get(prospectRef);
        ...
        if (!suppressedInterestLookup.complete || !suppressedQuoteLookup.complete) {
          throw new CrmConflictError('Cross-source suppression search could not be completed safely. Review the source records before promotion.', 409);
        }
        if (suppressedInterestLookup.collisions.length > 0 || suppressedQuoteLookup.collisions.length > 0) {
          throw new CrmConflictError('A matching source record is suppressed. Promotion cannot bypass do-not-contact.', 409);
        }
        if (identityLookup.collisions.some(({ document }) => isRecordSuppressed(document.data()))) {
          throw new CrmConflictError('A matching prospect is suppressed. Promotion cannot bypass do-not-contact.', 409);
        }
        if (identityLookup.collisions.length > 0) {
          throw new CrmConflictError('A matching prospect already exists. Review the existing record instead of duplicating it.', 409);
        }
    ...
    const mutationResult = await db.runTransaction(async (transaction) => {
      ...
      if (!prospect || prospect.userId !== owner.uid) {
        throw new CrmConflictError('Prospect record not found.', 404);
      }
      if (isRecordSuppressed(prospect)) {
        throw new CrmConflictError('This prospect is suppressed. CRM mutations are blocked.', 409);
      }
      if (input.action !== 'add_note' && isProspectContactBarrierActive(barrierSnapshot.data())) {
        throw new CrmConflictError('Prospect contact is globally blocked until unresolved suppression propagation is reconciled.', 409);
      }
      ...
      if (input.action === 'set_stage') {
        ...
        if (currentStage === 'reservation' || currentStage === 'paid') {
          throw new CrmConflictError('Reservation and paid stages are controlled by their verified workflows.', 409);
        }
        const status = safeProspectStatusForStage(input.stage);
        if (!status) throw new CrmConflictError('That stage is controlled by a verified reservation or payment event.', 409);
        if (input.stage === 'ready') {
          const gate = contactGate(prospect);
          if (!gate.allowed) {
            throw new CrmConflictError(`Qualification is incomplete: ${gate.missing.join(', ')}.`, 409);
          }
        }
        ...
        transaction.create(db.collection('activities').doc(), activityRecord(
          owner.uid,
          input.prospectId,
        ...
        transaction.create(db.collection('auditlog').doc(), auditRecord(owner.uid, 'crm.stage', input.prospectId, 'Owner changed a CRM stage on the source prospect record.'));
        return { status };
      }
      ...
      if (input.action === 'schedule_next_action') {
        ...
        transaction.create(db.collection('auditlog').doc(), auditRecord(owner.uid, 'crm.task_schedule', input.prospectId, 'Owner scheduled an internal next action. No outreach was sent.'));
        return {};
      }
      ...
      if (input.action === 'complete_next_action') {
        ...
        if (!currentAction && !currentDate) throw new CrmConflictError('No scheduled next action exists.', 409);
        ...
        transaction.create(db.collection('auditlog').doc(), auditRecord(owner.uid, 'crm.task_complete', input.prospectId, 'Owner completed an internal next action.'));
        return {};
      }
      ...
      transaction.update(prospectRef, { updatedAt: FieldValue.serverTimestamp() });
      transaction.create(db.collection('auditlog').doc(), auditRecord(owner.uid, 'crm.note', input.prospectId, 'Owner added an internal CRM note.'));
      return {};
    });
    return NextResponse.json({ success: true, ...mutationResult });
  } catch (error) {
...
function prospectView(id: string, data: DocumentData) {
  return {
    ...
    doNotContact: isRecordSuppressed(data),
...
function quoteProspectRecord(inquiry: DocumentData, ownerUid: string, inquiryId: string) {
  ...
  return {
    ...
    ...normalizedProspectIdentityFields({ businessName, email, phone, website: '' }),
    userId: ownerUid,
    createdAt: FieldValue.serverTimestamp(),
    updatedAt: FieldValue.serverTimestamp(),
  };
