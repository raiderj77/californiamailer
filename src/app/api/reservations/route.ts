...
import { LEGACY_RESERVATION_ACCESS_DAYS } from '@/lib/advertiserPortal';
...
export async function POST(request: NextRequest) {
  ...
  const legacyAccessVersion = 1;
  const legacyAccessExpiresAt = Timestamp.fromMillis(
    Date.now() + LEGACY_RESERVATION_ACCESS_DAYS * 24 * 60 * 60_000,
  );
  ...
  if (normalizedInvitationCode && (
    ...
    || invitation.emailNormalized !== normalizeEmail(parsed.data.email)
  ...
  if (!canCreatePaidHold) {
    const duplicateId = sha256(
      `${parsed.data.campaignId}:${normalizeEmail(parsed.data.email)}:${parsed.data.categorySlug}`,
    ).slice(0, 48);
  ...
  const duplicateRef = db
    ...
    .doc(sha256(`${parsed.data.campaignId}:${normalizeEmail(parsed.data.email)}`));
  ...
  try {
    reservedContractVersions = await db.runTransaction(async (transaction) => {
      ...
      if (!invite || invite.status !== 'active' || invite.reservationId
        ...
        || invite.emailNormalized !== normalizeEmail(parsed.data.email)
      ...
      if (typeof invite.createdBy !== 'string') throw new Error('invitation-required');
      const contactBarrier = await transaction.get(
        db.collection(PROSPECT_SUPPRESSION_STATE_COLLECTION).doc(invite.createdBy),
      );
      if (isProspectContactBarrierActive(contactBarrier.data())) throw new Error('invitation-required');
      ...
      if (
        ...
        || String(interest.suppressionPropagationStatus || '').startsWith('unresolved_')
        ...
        || interest.emailNormalized !== normalizeEmail(parsed.data.email)
      ...
      const linkedProspectId = validLinkedProspectId(interest.prospectId) ? interest.prospectId : '';
      if (!linkedProspectId || invite.prospectId !== linkedProspectId) throw new Error('invitation-required');
      const linkedProspect = await transaction.get(db.collection('prospects').doc(linkedProspectId));
      const linkedProspectData = linkedProspect.data();
      if (
        !linkedProspectData
        || linkedProspectData.userId !== invite.createdBy
        || isRecordSuppressed(linkedProspectData)
        || highConfidenceProspectIdentityMatches(interest, linkedProspectData).length === 0
      ) throw new Error('invitation-required');
      ...
      transaction.create(reservationRef, {
        ...
        legacyAccessExpiresAt,
        legacyAccessIssuedAt: FieldValue.serverTimestamp(),
  ...
  response.cookies.set(reservationCookieName(reservationRef.id), accessToken, {
    ...
    sameSite: 'lax',
    path: '/',
    expires: legacyAccessExpiresAt.toDate(),
  });
  return response;
}
