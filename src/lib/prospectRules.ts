...
export function normalizeBusinessName(value = '') {
  return value.trim().toLowerCase().replace(/\b(incorporated|corporation|company|limited|inc|corp|co|llc|ltd)\.?\b/g, '')
    .replace(/[^a-z0-9]/g, '');
}
...
export function contactGate(prospect: Partial<Prospect>) {
  ...
  if (prospect.doNotContact || prospect.status === 'do_not_contact') missing.push('do-not-contact suppression');
