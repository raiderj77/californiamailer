...
export const LEGACY_RESERVATION_ACCESS_DAYS = 90;
...
export function isActiveLegacyReservationAccess(
  reservation: DocumentData,
  now = Date.now(),
): boolean {
  const expiresAt = timestampMillis(reservation.legacyAccessExpiresAt);
  const accessVersion = reservationPortalAccessVersion(reservation);
  return reservation.legacyAccessStatus === 'active'
    && typeof reservation.accessTokenHash === 'string'
    && /^[a-f0-9]{64}$/.test(reservation.accessTokenHash.trim().toLowerCase())
    && Number.isSafeInteger(reservation.legacyAccessVersion)
    && reservation.legacyAccessVersion === accessVersion
    && expiresAt !== null
    && expiresAt > now;
}
