...
export const KNOWN_REFUND_RECORD_STATUSES = [
  'requested',
  'approved',
  'submitted',
  'confirmed',
...
export const ACTIVE_REFUND_OBLIGATION_STATUSES = [
  'requested',
  'approved',
  'submitted',
] as const;

export type RefundRecordStatus = typeof KNOWN_REFUND_RECORD_STATUSES[number];

const KNOWN_REFUND_RECORD_STATUS_SET = new Set<string>(KNOWN_REFUND_RECORD_STATUSES);
const ACTIVE_REFUND_OBLIGATION_STATUS_SET = new Set<string>(ACTIVE_REFUND_OBLIGATION_STATUSES);

export function isKnownRefundRecordStatus(value: unknown): value is RefundRecordStatus {
  return typeof value === 'string'
    && value === value.trim()
    && KNOWN_REFUND_RECORD_STATUS_SET.has(value);
}

export function isActiveRefundObligationStatus(value: unknown): boolean {
  return isKnownRefundRecordStatus(value)
    && ACTIVE_REFUND_OBLIGATION_STATUS_SET.has(value);
}
...
export interface ActiveRefundObligationSummary {
  activeCount: number;
  totalCents: number;
  integrityIssueCount: number;
}

const PROVIDER_IDENTIFIER_DISALLOWED_CHARACTERS = /[\s\u0000-\u001f\u007f-\u009f]/u;
const PROVIDER_IDENTIFIER_DISALLOWED_CHARACTERS_GLOBAL = /[\s\u0000-\u001f\u007f-\u009f]/gu;

export interface PaymentProviderIdentifierEvidence {
  externalPaymentId: string | null;
  externalPaymentIdCollisionCandidate: string | null;
  externalPaymentIdNoncanonical: boolean;
  checkoutSessionIds: string[];
  checkoutSessionIdCollisionCandidates: string[];
  checkoutSessionIdNoncanonical: boolean;
  checkoutSessionAliasMismatch: boolean;
}

export function canonicalPaymentProviderIdentifier(value: unknown): string | null {
  if (
    typeof value !== 'string'
    || value.length === 0
    || value !== value.trim()
    || PROVIDER_IDENTIFIER_DISALLOWED_CHARACTERS.test(value)
  ) {
    return null;
  }
  return value;
...
export function paymentProviderIdentifierCollisionCandidate(value: unknown): string | null {
  if (typeof value !== 'string') return null;
  const candidate = value.replace(PROVIDER_IDENTIFIER_DISALLOWED_CHARACTERS_GLOBAL, '');
  if (!candidate) return null;
  return candidate;
}

export function paymentProviderIdentifierEvidence(
  payment: Record<string, unknown>,
): PaymentProviderIdentifierEvidence {
  const externalPaymentIdValue = payment.externalPaymentId;
  const externalPaymentIdPresent = externalPaymentIdValue !== undefined
    && externalPaymentIdValue !== null;
  const externalPaymentId = canonicalPaymentProviderIdentifier(externalPaymentIdValue);
  const sessionValues = [
    payment.externalCheckoutSessionId,
    payment.externalSessionId,
  ].filter((value) => value !== undefined && value !== null);
  const checkoutSessionIds = [...new Set(sessionValues
    .map(canonicalPaymentProviderIdentifier)
    .filter((value): value is string => value !== null))];
  const checkoutSessionIdCollisionCandidates = [...new Set(sessionValues
    .map(paymentProviderIdentifierCollisionCandidate)
    .filter((value): value is string => value !== null))];
  return {
    externalPaymentId,
    externalPaymentIdCollisionCandidate:
      paymentProviderIdentifierCollisionCandidate(externalPaymentIdValue),
    externalPaymentIdNoncanonical: externalPaymentIdPresent && externalPaymentId === null,
    checkoutSessionIds,
    checkoutSessionIdCollisionCandidates,
    checkoutSessionIdNoncanonical: sessionValues.some((value) => (
      canonicalPaymentProviderIdentifier(value) === null
    )),
    checkoutSessionAliasMismatch: sessionValues.length > 1
      && sessionValues.some((value) => value !== sessionValues[0]),
  };
}

export function activeRefundObligationSummary(
  ...
  for (const refund of refunds) {
    if (!isActiveRefundObligationStatus(refund.status)) continue;
    activeCount += 1;
    const amountCents = refund.amountCents;
    const bindingValid = (!expectedCampaignId || refund.campaignId === expectedCampaignId)
      && typeof refund.paymentId === 'string'
      && Boolean(refund.paymentId.trim())
      && typeof refund.reservationId === 'string'
      && Boolean(refund.reservationId.trim());
    if (
      !bindingValid
      || !Number.isSafeInteger(amountCents)
      || Number(amountCents) <= 0
      || !Number.isSafeInteger(totalCents + Number(amountCents))
    ) {
      integrityIssueCount += 1;
      continue;
    }
    totalCents += Number(amountCents);
  }
  return { activeCount, totalCents, integrityIssueCount };
}
...
export function canonicalPaidPaymentEvidence(
  ...
  const paymentDocumentIds = new Set<string>();
  const externalPaymentIds = new Set<string>();
  const duplicateExternalPaymentIds = new Set<string>();
  const externalCheckoutSessionIds = new Set<string>();
  const duplicateExternalCheckoutSessionIds = new Set<string>();
  for (const payment of payments) {
    if (!payment.id || paymentDocumentIds.has(payment.id)) {
      issues.add('payment_document_id_duplicate_or_missing');
    } else {
      paymentDocumentIds.add(payment.id);
    }
    const providerIdentifiers = paymentProviderIdentifierEvidence(payment);
    if (providerIdentifiers.externalPaymentIdNoncanonical) {
      issues.add('payment_provider_id_noncanonical');
    }
    const externalPaymentId = providerIdentifiers.externalPaymentIdCollisionCandidate;
    if (externalPaymentId) {
      if (externalPaymentIds.has(externalPaymentId)) {
        issues.add('payment_provider_id_duplicate');
        duplicateExternalPaymentIds.add(externalPaymentId);
      }
      externalPaymentIds.add(externalPaymentId);
    }
    if (providerIdentifiers.checkoutSessionIdNoncanonical) {
      issues.add('payment_checkout_session_id_noncanonical');
    }
    if (providerIdentifiers.checkoutSessionAliasMismatch) {
      issues.add('payment_checkout_session_alias_mismatch');
    }
    for (const checkoutSessionId of providerIdentifiers.checkoutSessionIdCollisionCandidates) {
      if (externalCheckoutSessionIds.has(checkoutSessionId)) {
        issues.add('payment_checkout_session_id_duplicate');
        duplicateExternalCheckoutSessionIds.add(checkoutSessionId);
      }
      externalCheckoutSessionIds.add(checkoutSessionId);
    }
  ...
  for (const reservation of paidReservations) {
    ...
    const externalPaymentIdCollisionCandidate =
      providerIdentifiers.externalPaymentIdCollisionCandidate;
    const checkoutSessionIds = providerIdentifiers.checkoutSessionIds;
    ...
    const clearedEvidence = payment.status === 'cleared'
      ...
      && !duplicateExternalPaymentIds.has(externalPaymentIdCollisionCandidate || '')
      && !providerIdentifiers.checkoutSessionIdNoncanonical
      && !providerIdentifiers.checkoutSessionAliasMismatch
      && checkoutSessionIds.length === 1
      && providerIdentifiers.checkoutSessionIdCollisionCandidates.every((checkoutSessionId) => (
        !duplicateExternalCheckoutSessionIds.has(checkoutSessionId)
      ))
      && clearedAt > 0
...
export function hasReviewableCurrentCreativeBrief(
  ...
  if (
    ...
    || ![CREATIVE_BRIEF_PENDING_STATUS, CREATIVE_BRIEF_REVIEWED_STATUS].includes(
      creativeBrief.status as typeof CREATIVE_BRIEF_PENDING_STATUS,
    )
  ) {
    return false;
  }
...
export function hasCurrentCreativeBrief(
  ...
  return hasReviewableCurrentCreativeBrief(reservation, creativeBrief, campaign)
    && creativeBrief?.status === CREATIVE_BRIEF_REVIEWED_STATUS
    && creativeBrief.reviewConfirmation === PROOF_BRIEF_REVIEW_CONFIRMATION
    && typeof creativeBrief.reviewedBy === 'string'
    && Boolean(creativeBrief.reviewedBy.trim())
    && recordedTimestampMillis(creativeBrief.reviewedAt) > 0
    && recordedTimestampMillis(creativeBrief.reviewedAt) <= now.getTime();
}

export interface CurrentMaterialBinding {
  assetKind: CreativeAssetKind;
  materialId: string;
  materialVersion: number;
  material: PrivateCreativeRecord;
}

function consistentCurrentManifestPointers(reservation: PrivateCreativeRecord) {
  ...
  const latestMaterialId = reservation.latestMaterialId;
  const materialSequence = reservation.materialSequence;
  if (
    typeof latestMaterialId !== 'string'
    || !latestMaterialId
    || !Number.isSafeInteger(materialSequence)
    || Number(materialSequence) < 1
    || Math.max(...pointers.map(({ version }) => version)) !== Number(materialSequence)
    || pointers.filter((pointer) => (
      pointer.materialId === latestMaterialId
      && pointer.version === materialSequence
    )).length !== 1
  ) {
    return null;
  }
...
function exactMaterialBinding(
  reservation: PrivateCreativeRecord,
  material: PrivateCreativeRecord | undefined,
  assetKind: CreativeAssetKind,
  materialId: string,
  materialVersion: number,
): CurrentMaterialBinding | null {
  if (
    !material
    ...
    || material.assetKind !== assetKind
    || material.version !== materialVersion
  ) {
    return null;
  }
  return { assetKind, materialId, materialVersion, material };
}

export function currentMaterialBindings(
  reservation: PrivateCreativeRecord,
  materialsInput: PrivateCreativeRecord | PrivateCreativeRecord[] | undefined,
): CurrentMaterialBinding[] | null {
  const materials = Array.isArray(materialsInput)
    ? materialsInput
    : materialsInput
      ? [materialsInput]
      : [];
  if (reservation.materialManifest !== undefined && reservation.materialManifest !== null) {
    const pointers = consistentCurrentManifestPointers(reservation);
    if (!pointers) return null;
    const bindings: CurrentMaterialBinding[] = [];
    for (const pointer of pointers) {
      const material = materials.find((candidate) => candidate.id === pointer.materialId);
      const binding = exactMaterialBinding(
        reservation,
        material,
        pointer.assetKind,
        pointer.materialId,
  ...
  const materialVersion = reservation.materialSequence;
  const material = materials.find((candidate) => candidate.id === materialId);
  const assetKind = material?.assetKind;
  if (
    typeof materialId !== 'string'
    || !materialId
    || !Number.isSafeInteger(materialVersion)
    || Number(materialVersion) < 1
    || !CREATIVE_ASSET_KINDS.includes(assetKind as CreativeAssetKind)
  ) {
    return null;
  }
  const binding = exactMaterialBinding(
    reservation,
    material,
    assetKind as CreativeAssetKind,
    materialId,
    Number(materialVersion),
  );
...
export function hasCurrentApprovedMaterialWithRights(
  reservation: PrivateCreativeRecord,
  materials: PrivateCreativeRecord | PrivateCreativeRecord[] | undefined,
  now = new Date(),
): boolean {
  const bindings = currentMaterialBindings(reservation, materials);
  if (!bindings) return false;
  return bindings.every(({ material }) => {
    if (
      material.status !== 'owner_approved_private'
...
function proofMaterialsMatch(reservation: PrivateCreativeRecord, proof: PrivateCreativeRecord) {
  if (reservation.materialManifest !== undefined && reservation.materialManifest !== null) {
    const expected = consistentCurrentManifestPointers(reservation);
    if (!expected || !Array.isArray(proof.materialBindings)) return false;
    const actualBindings = proof.materialBindings as unknown[];
    if (actualBindings.length !== expected.length) return false;
    return expected.every((pointer, index) => {
      const binding = actualBindings[index];
      return typeof binding === 'object'
        && binding !== null
        && !Array.isArray(binding)
        && (binding as Record<string, unknown>).assetKind === pointer.assetKind
        && (binding as Record<string, unknown>).materialId === pointer.materialId
        && (binding as Record<string, unknown>).materialVersion === pointer.version;
    });
  }
  return proof.materialId === reservation.latestMaterialId
...
export function latestBoundProofStatus(
  ...
  if (
    ...
    || !proofMaterialsMatch(reservation, proof)
  ) {
...
export interface PrintReadinessInput {
  ...
  refundObligationCount?: number;
  refundObligationIntegrityIssueCount?: number;
...
export function evaluatePrintReadiness(input: PrintReadinessInput): {
  ...
  const refundObligationCount = input.refundObligationCount
    ?? (input.refundObligationCents === 0 ? 0 : 1);
  const refundObligationIntegrityIssueCount = input.refundObligationIntegrityIssueCount ?? 0;
  ...
  const checks: ReadinessCheck[] = [
    ...
    },
    {
      ...
      passed: input.refundObligationCents === 0
        && refundObligationCount === 0
        && refundObligationIntegrityIssueCount === 0,
      detail: refundObligationIntegrityIssueCount > 0
        ? `${refundObligationIntegrityIssueCount} active refund record(s) have invalid amount or binding evidence`
        : `${input.refundObligationCents} cents across ${refundObligationCount} active obligation(s) awaiting resolution`,
    },
    {
      key: 'payment_reviews',
