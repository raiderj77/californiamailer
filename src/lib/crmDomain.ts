...
export function buildCrmOpportunities(
  ...
  const prospectOpportunities = prospects.map((prospect): CrmOpportunity => {
    ...
    return {
      ...
      doNotContact: isRecordSuppressed(prospect),
  ...
  const interestOpportunities = interests
    ...
    .map((interest): CrmOpportunity => {
      ...
      return {
        ...
        stage: isRecordSuppressed(interest) ? 'closed' : interestStatusToCrmStage(interest.status),
        ...
        doNotContact: isRecordSuppressed(interest),
        nextActionDate: '',
        nextActionType: '',
        nextActionNote: '',
  ...
  const quoteOpportunities = quoteInquiries
    ...
    .map((inquiry): CrmOpportunity => {
      ...
      const closed = clean(inquiry.status) === 'dismissed' || isRecordSuppressed(inquiry);
      return {
        id: `quote:${inquiry.id}`,
        recordId: inquiry.id,
        ...
        doNotContact: isRecordSuppressed(inquiry),
        nextActionDate: '',
        nextActionType: '',
        nextActionNote: '',
        taskState: 'unscheduled',
