...
**Release state:** implemented in the current integration tree on branch `codex/faceless-sales`. Draft PR #3 is open; the exact current PR head and its live GitHub Actions checks are the review authority. Nothing was deployed or externally activated. No outreach was sent, no real advertiser/subscriber/payment records were created, no checkout was enabled, no provider was activated, and no print, postage, or restaurant-distribution order was made.
...
- the automated business/security suite passing;
- clean TypeScript and repository-wide ESLint;
- a supported webpack production build (`npx next build --webpack`) completing static generation;
...
- the default Turbopack `npm run build` is environment-blocked only in this writable clone because its temporary `node_modules` junction points outside Turbopack's filesystem root; GitHub Actions uses a normal in-tree `npm ci` install and is authoritative for the exact pushed commit's default build;
...
- a `.github/workflows/ci.yml` using Node 24, least permissions, concurrency cancellation, and full-SHA action pins. Draft PR #3 is open, and its live checks are the authority for the current head;
...
- GitHub `main` was unprotected at the August 20 audit. Draft PR #3 is open; its exact current-head checks are visible on GitHub, and nothing has been merged or deployed.
