...
CaliforniaMailer is an existing Next.js 16 / React 19 application using Firebase Authentication, Firestore, Stripe and Mailgun route handlers, Tailwind, Vercel, and Node's test runner. Work is performed from the repository root on branch `codex/faceless-sales`, with the maintained repository configured as its local origin.
...
- Keep the GitHub Actions workflow least-privileged and deterministic with Node 24, `npm ci`, the full test suite, TypeScript, lint, and production build. Draft PR #3 is open, and its live checks are authoritative for the exact pushed head.
...
The implementation sequence above is represented in the current integration tree. Release validation covers the automated test suite, TypeScript, repository-wide ESLint, an offline dependency audit, a supported webpack production build, sitemap checks, and responsive browser QA. The default Turbopack build is environment-blocked only in the writable clone because its temporary `node_modules` junction points outside Turbopack's filesystem root; GitHub Actions uses a normal in-tree `npm ci` install and is authoritative for the exact pushed commit's default build. An online advisory refresh remains a release-time check. A React development warning on `/quote` was traced precisely to browser-extension-injected `fdprocessedid` attributes rather than source divergence.

The provider snapshot is intentionally narrower than local validation. Vercel project identity, the production baseline deployment, and environment-variable names were verified read-only; the new branch and routes were not deployed. Firebase's local project alias, database location, and checked-in indexes were identified, but expired CLI authentication left the live app, deployed rules/indexes, IAM, billing, and data state unverified. Draft PR #3 is open; its exact current-head checks are visible on GitHub, and nothing has been merged or deployed.
