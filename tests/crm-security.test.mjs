...
test('CRM reuses source collections and protects payment-backed stages', () => {
  ...
  const identityServer = read('src/lib/prospectIdentityServer.ts');
  ...
  assert.match(route, /queryProspectIdentityCollisions\(/);
  assert.match(identityServer, /where\(spec\.field, '==', spec\.value\)/);
  ...
  assert.match(quoteStatus, /isRecordSuppressed\(inquiry\)/);
...
test('CRM mutations re-read suppression inside transactions and expose all DNC markers to the UI', () => {
  const route = read('src/app/api/admin/crm/route.ts');
  const domain = read('src/lib/crmDomain.ts');
  const page = read('src/app/(dashboard)/crm/page.tsx');
  const prospectMutation = route.slice(route.indexOf("const prospectRef = db.collection('prospects').doc(input.prospectId)"), route.indexOf('function prospectView'));

  assert.match(prospectMutation, /db\.runTransaction\(async \(transaction\) =>/);
  assert.match(prospectMutation, /await transaction\.get\(prospectRef\)/);
  assert.match(prospectMutation, /isRecordSuppressed\(prospect\)/);
  assert.match(route, /clean\(inquiry\.status\) === 'dismissed' \|\| isRecordSuppressed\(inquiry\)/);
  assert.match(route, /doNotContact: isRecordSuppressed\(data\)/);
  assert.match(domain, /doNotContact: isRecordSuppressed\(interest\)/);
  assert.match(domain, /doNotContact: isRecordSuppressed\(inquiry\)/);
  assert.ok((page.match(/item\.doNotContact \|\| item\.sourceStatus === 'dismissed'/g) || []).length >= 3);
  assert.match(page, /busy \|\| contactGloballyBlocked \|\| item\.doNotContact/);
});
...
test('legacy prospect operational notes cannot manufacture sold or paid workflow state', () => {
  ...
  const mutationRoute = read('src/app/api/admin/prospects/route.ts');
  ...
  assert.match(rules, /match \/prospects\/\{docId\}[\s\S]*allow create: if false/);
  assert.match(rules, /&& preservesOrClearsLegacyProspectState\(\)/);
  assert.match(mutationRoute, /A new prospect cannot claim a legacy operational status/);
  assert.match(mutationRoute, /New prospect records cannot claim reservation or payment state/);
  assert.match(mutationRoute, /Legacy operational prospect states can be preserved or cleared, but not newly asserted/);
});
