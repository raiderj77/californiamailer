...
test('proof, material, and refund mutations atomically revoke cached print readiness', () => {
  ...
  assert.match(material, /transaction\.update\(db\.collection\('campaigns'\)\.doc\(material\.campaignId\), \{[\s\S]*ownerPrintApproved: false,[\s\S]*printReadyAt: null,[\s\S]*artworkPreflightApproved: false/);
  assert.match(material, /printReadinessRevokedReason: `material_\$\{parsed\.data\.action\}`/);
  assert.match(materialUpload, /transaction\.update\(db\.collection\('campaigns'\)\.doc\(currentReservation\.campaignId\), \{[\s\S]*ownerPrintApproved: false,[\s\S]*printReadyAt: null,[\s\S]*artworkPreflightApproved: false/);
...
test('print scheduling consumes only current transactionally revalidated readiness', () => {
  ...
  assert.match(route, /latest\.ownerPrintApproved !== true \|\| !latest\.printReadyAt/);
  assert.match(route, /\['schedule_for_print', 'record_printed'\]\.includes\(action\)/);
  assert.match(route, /transaction\.update\(campaignRef, update\)/);
  ...
  assert.match(economics, /transaction\.get\(db\.collection\('refunds'\)\.where\('campaignId', '==', FOUNDING_CAMPAIGN\.id\)\)/);
...
test('every consequential campaign lifecycle transition re-reads current state transactionally', () => {
  ...
  assert.match(route, /transaction\.get\(db\.collection\('refunds'\)\.where\('campaignId', '==', FOUNDING_CAMPAIGN\.id\)\)/);
});
