...
test('every new material version stores explicit rights and atomically revokes print readiness', () => {
  ...
  assert.match(readPaid, /materialSequence\(access\.data\.materialSequence\)/);
  assert.match(readPaid, /assertMaterialPointer\(access\.data, sequence\)/);
  assert.match(readPaid, /transaction\.get\(db\.collection\('materials'\)\.doc\(latestId\)\)/);
  ...
  assert.match(transaction, /assertMaterialPointer\(currentReservation, storedSequence\)/);
  assert.match(transaction, /transaction\.get\(db\.collection\('materials'\)\.doc\(latestId\)\)/);
  assert.match(transaction, /previousMaterialId: latestId/);
